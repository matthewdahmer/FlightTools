...
fileparts = {'psmc':'_1pdeaat_plot.txt',
             'minusz':'_tephin_plot.txt',
             'dpa':'_dpa_plot.txt',
             'tank':'_pftank2t_plot.txt',
             'mups':'_mups_valves_plot.txt'}

headerinfo = {'psmc':{'columns':3,
                      'names':['Time', '1PDEAAT', '1PIN1AT'],
                      'title':'ACIS: PSMC'},
              'minusz':{'columns':8,
                        'names':['Time','TEPHIN', 'TCYLAFT6', 'TMZP_MY',
                                 'TCYLFMZM', 'TFSSBKT1', 'Pitch', 'Roll'],
                        'title':'Spacecraft: Minus-Z'},
              'dpa':{'columns':9,
                     'names':['Time','1DPAMZT', 'Pitch', 'Roll', 'SimPos',
                              'FEP_Count', 'CCD_Count', 'Vid_Board', 'Clocking'],
                     'title':'ACIS: DPA'},
              'tank':{'columns':6,
                      'names':['Time', 'PFTANK2T', 'PFTANKIP', 'PF0TANK2T',
                               'Pitch', 'Roll'],
                      'title':'Spacecraft: Fuel Tank'},
              'mups':{'columns':5,
                      'names':['Time', 'PM1THV1T', 'PM1THV1T Settle',
                               'PM2THV1T','PM2THV1T Settle'],
                      'title':'Spacecraft: MUPS Valves'}}

plotorder = ['minusz', 'tank', 'mups', 'psmc', 'dpa']


def __readfile(filename,subheaderinfo):

    # Headerinfo should be a sub-dict of the original header info, including
    # only the information for the current model
...
if __name__ == "__main__":
    ...
    if args['OutputPropEndingConfiguration']:
        if args['Propschedule'] == None:
            print('Error: No propagation schedule name entered')
        else:
            propschedulename = os.path.basename(args['Propschedule'])
            writePropData(args['Propschedule'])
            print('Wrote propagation ending data to '\
                  '%s_Ending_Configuration.txt\n'%propschedulename)

    if args['OutputThermalReport']:
        if (args['Propschedule'] == None) | (args['Reviewschedule'] == None):
            print('Error: No propagation and/or review schedule names entered')
        else:
            reviewschedulename = os.path.basename(args['Reviewschedule'])
            writeChecklistData(args['Propschedule'],args['Reviewschedule'])   
